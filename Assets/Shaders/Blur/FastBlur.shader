...
Shader "Kitte/FastBlur" {
	...
	v2f vertComposite(appdata_img v)
	{
		v2f o;
		o.pos = UnityObjectToClipPos(v.vertex);
	...
	half4 fragBlurComposite(v2f i) : SV_Target{
		half4 screencolor = tex2D(_MainTex, i.uv[0].xy);
		half4 blurcolor = tex2D(_BlurTex, i.uv[1].xy);
		return lerp(screencolor, blurcolor, saturate(_Intensity));
	}
	ENDCG
	...
	SubShader {
		...
		// 8 : Full-screen blur composite
		Pass {
			CGPROGRAM
			#pragma vertex vertComposite
			#pragma fragment fragBlurComposite
			ENDCG
		}
	}
	FallBack Off
