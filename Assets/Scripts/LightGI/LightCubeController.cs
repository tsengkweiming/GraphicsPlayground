...
/// <summary>
/// Drives the LightCube 2-D GI compute effect.
///
/// OnRenderImage is not called in URP, so we hook into
/// RenderPipelineManager.endCameraRendering instead — same idea,
/// one extra line of setup.
/// </summary>
///
[System.Serializable]
public class LightCubeParam
{
    [Range(1,20)] public int MaxRay;
    public int MaxStep;
    [Range(0,3000)] public int MaxDist;
    public float AttenuationPower;
    [Range(1,20)] public int CubeCount;
...
public class LightCubeController : MonoBehaviour
{
    ...
    [SerializeField] private LightCubeParam _lightCubeParam;
    ...
    private void OnEndCameraRendering(ScriptableRenderContext ctx, Camera cam)
    {
        ...
        _lightCompute.SetInt   ("_Frame",      _frameCount++);
        _lightCompute.SetInt   ("_MaxRay",      _lightCubeParam.MaxRay);
        _lightCompute.SetInt   ("_MaxStep",      _lightCubeParam.MaxStep);
        _lightCompute.SetInt   ("_MaxDist",      _lightCubeParam.MaxDist);
        _lightCompute.SetInt   ("_CubeCount",      _lightCubeParam.CubeCount);
        _lightCompute.SetFloat ("_AttenuationPower",  _lightCubeParam.AttenuationPower);
        _lightCompute.SetFloat ("_EdgeThickness",     _lightCubeParam.EdgeThickness);
        _lightCompute.SetFloat ("_OrbitAngularSpeed", _lightCubeParam.OrbitAngularSpeed);
        _lightCompute.SetFloat ("_RotateXMultiplier", _lightCubeParam.RotateXMultiplier);
        _lightCompute.SetFloat ("_RotateYMultiplier", _lightCubeParam.RotateYMultiplier);
        ...
        var cmd = CommandBufferPool.Get("LightCubeBlit");
